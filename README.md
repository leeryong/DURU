...
<div align="center">
  <img src="assets/duru_agents.png" alt="DURU의 에이전트들 — 기본 비서 별이, 행정 도우미 하루, 일 파트너 미로, 공부 친구 토토, 수업 도우미 초코. 문서를 지식베이스로 담아 내 PC 안에서 안전하게 쓴다" width="84%" />
  <br />
  <sub>역할이 다른 다섯 비서 — 별이 · 하루 · 미로 · 토토 · 초코</sub>
</div>

---

> ## 📢 윈도우·맥 설치판이 나왔습니다 — **v0.8.5**
>
> <sub>2026-09-02 · 안정성 개선과 Ollama 자동 설치</sub>
>
> - **처음 켜면 스스로 갖춥니다** — Ollama 가 없으면 받아 설치하고, 그 PC 의 GPU·메모리에 맞는 대화 모델까지 골라 받습니다.
> - **안정성을 손봤습니다** — 근거가 있는데도 「없습니다」라고 하던 답과, 문서 파서 GPU 가속이 켜지지 않던 문제를 고쳤습니다.
>
> 설치 파일 하나(약 262MB)를 받아 실행하면 끝입니다. → **[내려받기](../../releases/latest)**
...
>
> 🍎 **맥(Apple Silicon)판도 함께 공개했습니다.** `.dmg`(약 299MB)를 열어 DURU 를
> `응용 프로그램` 으로 끌어 넣으면 됩니다. macOS 14 이상 · M1 이상.
> **애플 공증을 마쳐 경고 없이 바로 열립니다.** → **[내려받기](https://github.com/leeryong/DURU/releases/tag/v0.8.5)**
...
교재를 지식베이스로 담아 두면, **공부 친구 토토**가 그 교재에서 문제를 내고 답을 봐 줍니다.
아래는 영문법 교재로 실제로 해 본 모습입니다.

> ⚠️ **교재는 들어 있지 않습니다.** 화면에 보이는 책들은 예를 보이기 위한 것이고,
...
<div align="center">
  <img src="assets/example-english-quiz.jpg" alt="DURU 화면 — 왼쪽에 영어 교육자료 지식베이스, 가운데 뷰어에 영문법 교재 111쪽, 오른쪽에서 토토가 그 쪽 내용으로 빈칸 문제 세 개를 냈다" width="94%" />
</div>
...
<div align="center">
  <img src="assets/example-english-feedback.jpg" alt="사용자가 doesn't, did, do 라고 답하자 토토가 항목마다 정답 여부를 표시하고 틀린 것은 이유를 설명하는 화면" width="94%" />
</div>

<sub>답을 적어 보내면 문항마다 ✅ / ❌ 로 채점하고, <b>틀린 까닭</b>을 교재의 표현으로 설명합니다.
위 예에서는 세 번째 답을 <code>do</code> 라고 냈는데, 강조 용법이라 <code>did</code> 가 맞다는 것을
원문 <i>"Your sister <b>did do</b> well…"</i> 을 짚어 알려 줍니다.
마지막에는 배운 것을 써먹어 보는 확인 질문을 하나 덧붙입니다.</sub>

**이렇게 쓸 수 있습니다**

- 교재를 바꾸면 그대로 다른 과목 공부가 됩니다 — 자격증 교재, 전공 서적, 사내 교육자료
- 인터넷이 없어도 됩니다. 교재도 대화도 **내 PC 안에서만** 오갑니다
- 답이 교재 밖으로 새지 않습니다. 근거를 눌러 원문에서 바로 확인할 수 있습니다

<details>
<summary>📚 예시 화면에 쓰인 교재 (모두 저작권자에게 권리가 있습니다)</summary>
...
| | |
| --- | --- |
| **DURU-Setup-0.8.5.exe** | 윈도우 10/11 (64비트) · 약 262MB · [내려받기](../../releases/latest) |
| **DURU-0.8.5-macOS-arm64.dmg** | macOS 14 이상 · Apple Silicon(M1 이상) · 약 299MB · [내려받기](../../releases/latest) |

설치본 안에 **맛보기 지식베이스 「국가 제도·법령 (표본 15편)」이 들어 있습니다.** 첫 실행 때
한 번 자동으로 준비되므로 따로 받거나 가져올 필요가 없습니다.
남에게 건네고 싶으면 DURU 안에서 **내보내기**를 눌러 `.durukb` 파일로 만들 수 있습니다.
...
> [!NOTE]
> 🍎 **맥(macOS)판도 공개했습니다** — [v0.8.5 릴리즈](https://github.com/leeryong/DURU/releases/tag/v0.8.5).
> Apple Silicon(M1 이상) · macOS 14 이상 전용입니다. **애플 공증을 마쳤으므로**
> 경고 없이 바로 열립니다.
> 쓰는 방법은 윈도우와 같고, 설치 파일(`.dmg`)을 열어 DURU 를 `응용 프로그램` 폴더로
> 끌어다 놓는 것만 다릅니다.
> 처음 켜면 문서 파서와 한글·오피스 리더기를 스스로 내려받습니다(약 1GB, 몇 분).
